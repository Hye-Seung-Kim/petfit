...
A shared AI virtual fitting room for pet outfits. A host publishes a
prerecorded dog video into a Vonage Video session; a friend joins the same
room and watches it live; the host captures a frame, picks a demo outfit,
and generates an AI try-on preview with Gemini; both participants see the
result and the friend can vote on their favorite look.

Built on top of Vonage's [Video Node.js learning-server starter](#appendix-vonage-video-nodejs-learning-server-starter-original-docs)
(kept intact below) plus a Node/Express + vanilla-JS layer for the fitting
room itself, and a standalone Python research spike that scoped out the
Gemini side before it was wired into the app - see
...
| Path | What it is |
|---|---|
| `app.js`, `routes/index.js`, `views/`, ... | The original Vonage starter (Vonage session/token routes, archiving, SIP, captions). Untouched - see the [appendix](#appendix-vonage-video-nodejs-learning-server-starter-original-docs). |
| `routes/fitting.js`, `services/*.js`, `public/fitting/`, `public/outfits/`, `test/roomState.test.js` | **PawTry Together** itself: the fitting-room page, JSON API, Gemini call, and client. |
| [`work/gemini-pet-tryon-spike/`](work/gemini-pet-tryon-spike/README.md) | A standalone **Python research spike** (separate from the running app) that compared Gemini image models and prompt strategies for pet try-on before this feature was built. Summary below. |
...
documented in the appendix to get `{ applicationId, sessionId, token }` for
Vonage.

**Added:**
- `services/roomState.js` - in-memory room state + mutations, including
  reset (tested, see below)
- `services/outfits.js` - the fixed 3-outfit demo catalog
- `services/gemini.js` - Gemini image-edit call, with a mock mode
- `routes/fitting.js` - the `/fitting/:room` page route and the
  `/api/rooms/*` (`state`, `generate`, `vote`, `finalize`, `reset`) +
  `/api/config` JSON API
- `public/fitting/` - the vanilla JS/CSS client (OpenTok.js from the
  official CDN, no build step)
- `public/outfits/*.svg` - 3 hand-drawn placeholder outfit icons (clearly
  labeled demo assets, not real product photos)
- `public/demo/README.txt` - where to drop your demo video
- `test/roomState.test.js` - unit tests for the room-state mutation logic

**Modified (minimally):**
- `app.js` - mounts the new router, and raises the global JSON body limit
  from express's 100kb default to 15mb so a captured video frame (sent as
  base64 JSON) fits. Nothing about the existing routes' behavior changed.
...
GEMINI_IMAGE_MODEL=gemini-2.5-flash-image   # override if needed - see the model note below
DEMO_MOCK_GEMINI=false                      # set true to skip real Gemini calls
```

> **Model note:** the app currently defaults `GEMINI_IMAGE_MODEL` to
> `gemini-2.5-flash-image`. The research spike below independently evaluated
> model choice afterwards and recommends `gemini-3.1-flash-image` instead
> for better identity preservation - see
> [Research: which Gemini model to use](#research-which-gemini-model-to-use).
> That newer default hasn't been adopted in `services/gemini.js` yet; you
> can try it today by setting `GEMINI_IMAGE_MODEL=gemini-3.1-flash-image` in
> `.env` without any code changes.
...
6. Pick an outfit card (Red Hoodie / Yellow Raincoat / Formal Tuxedo), then
   ...
   disabled while a request is in flight so you can't double-submit. If the
   AI doesn't respond within 45s, the client aborts and shows a clear
...
11. To run another round in the same room without restarting the server,
    click **Reset Room (clear looks & votes)** in window A (host-only, asks
    for confirmation) - it clears looks/votes/final pick for everyone and
    deletes the old generated image files.
...
## Research: which Gemini model to use

Before wiring Gemini into the app, [`work/gemini-pet-tryon-spike/`](work/gemini-pet-tryon-spike/README.md)
did a focused, standalone Python spike (its own script, prompts, and
evaluation sheet - not part of the running server) to answer: which Gemini
image model and prompt strategy best preserve a dog's identity when adding
an outfit?

**Verdict: PARTIAL.** The request/prompt workflow, retry policy, and
evaluation rubric are fully built, but visual quality was **not scored**
in that environment - no `GEMINI_API_KEY` or approved dog image was
available at spike time. The recommendation below is reasoning- and
documentation-based, not measured.

Key findings adopted (or worth adopting) into the main app:

- **Model:** recommends `gemini-3.1-flash-image` over the
  `gemini-2.5-flash-image` this app currently defaults to, for better
  identity/consistency (see the model note in [Setup](#setup)).
- **Prompt structure:** source dog image first, optional outfit-reference
  image second (labeled as garment cues only), instruction prompt last -
  matches this app's `services/gemini.js` ordering.
- **Response mode:** request image-only output to avoid getting
  explanatory text back instead of an image - already done here.
- **Retry policy:** retry HTTP 429 and transient 5xx up to 3 times with
  backoff; never retry auth/validation/policy errors; never silently
  substitute a cached/mock image for a failed real request.
- **8 documented failure modes** to watch for in real output: identity
  drift, fur/marking drift, anatomy errors, garment geometry errors, scene
  drift, reference-image leakage, over-styling, and outright format
  failures (see the spike README for detail).
- **Demo Mode discipline:** real API and demo/mock output must be two
  explicit, clearly-labeled paths that never silently fall into each
  other - the same principle this app's `DEMO_MOCK_GEMINI` flag follows.
- **Disclaimer language:** "creative style preview, not a physical sizing
  or fit guarantee" - matches the caption already shown under every
  generated look in this app's UI.

See [`work/gemini-pet-tryon-spike/README.md`](work/gemini-pet-tryon-spike/README.md)
for the full write-up, the 9-run prompt/outfit comparison in `prompts.md`,
and the evaluation rubric in `evaluation.md`.

## What's actually verified

This went through two rounds of verification:

**Mock-mode / no real credentials** (first pass, in an environment with no
Vonage or Gemini credentials available):
- `npm install` and `npm start` boot cleanly (after the dependency fix above).
- `npm test` - all room-state unit tests pass (12, including reset).
- `GET /room/:name` reaches the real Vonage API and correctly gets rejected
  with 401 for throwaway fake credentials (proves the JWT signing / request
  path works).
- `GET /api/config`, `GET /api/rooms/:room/state` - correct shapes.
- Full `generate` (mock mode) → `vote` → `finalize` → `reset` → re-fetch
  `/state` happy path, plus validation errors for a bad `outfitId`, a
  missing frame, and voting/finalizing a nonexistent look.
- `/fitting/:room` page and static assets serve correctly; an invalid room
  name is rejected with 400.

**With real credentials** (second pass, after the user supplied a real
Vonage application + a real Gemini API key):
- `GET /room/:name` against the real Vonage API returned a valid session ID
  and signed JWT.
- `POST /api/rooms/:room/generate` with `DEMO_MOCK_GEMINI=false` made a real
  `gemini-2.5-flash-image` call and returned a genuine generated PNG
  (verified via `file` - real image data, not a stub).
- The user then ran the actual two-browser flow (host publish, friend
  subscribe, live signal-triggered state refresh, vote, finalize) manually
  in Chrome and reported it working, including a real UI bug (a duplicate
  unstyled Vonage publisher preview) that's now fixed in `fitting.css`.

**Still not independently verified by an automated check:** cross-browser /
mobile behavior (desktop Chrome only, per the brief), and the
`gemini-3.1-flash-image` model recommendation from the research spike
(the app still defaults to `gemini-2.5-flash-image`, which is what was
actually exercised above).

---

## Appendix: Vonage Video Node.js learning-server starter (original docs)

<details>
<summary>Original Vonage starter README - session/token routes, archiving, SIP, captions, deployment</summary>
